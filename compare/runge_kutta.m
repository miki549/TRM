function tau = runge_kutta(S, T, t, X, mu, k_stoch_func, reaction_matrix, cap, Nc, tolerance)
    ...
    while integral_sum < (S - T)
        
        % RK4 lépés végrehajtása és hibabecslés
        [increment, error_est] = rk4_step(t, tau, X, mu, h, k_stoch_func, reaction_matrix, cap, Nc);
        ...
        integral_sum = integral_sum + increment;
        tau = tau + h;
        ...
        if integral_sum > (S - T)
            ...
            tau = tau - h * (excess/increment);  % Arányos visszalépés
            break;
        end
    end
